...
class OrderUserController extends Controller
{
    public function index()
	{
		...
		return view('user.frontend.orders.orders', compact('orders'));
	}
	...
	public function show($id)
	{
		...
		return view('user.frontend.orders.show',compact('order'));
	}
    ...
    public function checkout()
    {
		...
		return view('user.frontend.orders.checkout', compact('items','totalWeight','provinces','cities'));
	}
    ...
    private function _saveOrder($params)
	{
        ...
        if ($user instanceof \App\Models\User) {
            ...
            $user->city_id = $params['shipping_city_id'];
	...
	private function _saveOrderItems($order)
	{
		...
		if ($order && $cartItems) {
			foreach ($cartItems as $item) {
				...
				$orderItem = OrderItems::create($orderItemParams);

				if ($orderItem) {
					ProductInventories::reduceStock($orderItem->product_id, $orderItem->qty);
				}
			}
		}
	}
	...
	private function _generatePaymentToken($order)
	{
		...
		$params = [
			'enable_payments' => Payments::PAYMENT_CHANNELS,
			'transaction_details' => [
				'order_id' => $order->code,
			...
			'expiry' => [
				...
				'unit' => Payments::EXPIRY_UNIT,
				'duration' => Payments::EXPIRY_DURATION,
			],
		];
	...
	private function _saveShipment($order, $params)
	{
		...
		Shipments::create($shipmentParams);
	}
	...
	public function received($orderId)
	{
		...
		return view('user.frontend.orders.received', compact('order'));
	}
