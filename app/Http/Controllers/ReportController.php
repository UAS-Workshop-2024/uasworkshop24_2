...
class ReportController extends Controller
{
    ...
    ORDER BY DR.date ASC";
        ...
        //         $pdf = PDF::loadView('frontendadmin.reports.exports.pdf_revenue', compact('revenues','startDate','endDate'));
        ...
        if ($exportAs = $request->input('export')) {
            if (!in_array($exportAs, ['pdf'])) {
                // Menangani jika request export tidak valid (hanya menerima 'pdf')
                return redirect('reports/revenue');
            }
            ...
            if ($exportAs == 'pdf') {
                ...
                // Membuat PDF dengan data yang sudah disiapkan
                $pdf = PDF::loadView('frontendadmin.reports.exports.pdf_revenue', compact('revenues', 'startDate', 'endDate'));
        ...
        return view('frontendadmin.reports.revenue', compact('revenues','exports'));
    }
    ...
    public function product(Request $request)
	{
        ...
        if ($exportAs = $request->input('export')) {
            ...
            if ($exportAs !== 'pdf') {
                // Jika tidak 'pdf', redirect kembali
                return redirect('reports/product');
            }

            // Jika permintaan adalah 'pdf', proses untuk membuat file PDF
            if ($exportAs == 'pdf') {
                $fileName = 'report-product-' . $startDate . '-' . $endDate . '.pdf';

                // Membuat PDF dengan data yang sudah disiapkan
                $pdf = PDF::loadView('frontendadmin.reports.exports.pdf_product', compact('products', 'startDate', 'endDate'));
		...
		return view('frontendadmin.reports.product', compact('products', 'exports'));
    }
    ...
    public function inventory(Request $request)
	{
        ...
        if ($exportAs = $request->input('export')) {
            if ($exportAs !== 'pdf') {
                // Hanya ekspor PDF yang diizinkan
                return redirect('reports/inventory');
            }

            // Ekspor PDF
            $fileName = 'report-inventory.pdf';
            $pdf = PDF::loadView('frontendadmin.reports.exports.pdf_inventory', compact('products'));

            return $pdf->download($fileName);
        }

		return view('frontendadmin.reports.inventory', compact('products', 'exports'));
    }
    ...
    public function payment(Request $request)
	{
        ...
        if ($exportAs = $request->input('export')) {
            ...
            // Ekspor PDF
            $fileName = 'report-payment-' . $startDate . '-' . $endDate . '.pdf';
            $pdf = PDF::loadView('frontendadmin.reports.exports.pdf_payment', compact('payments', 'startDate', 'endDate'));

            return $pdf->download($fileName);
        }

		return view('frontendadmin.reports.payment', compact('payments', 'exports'));
	}
}
